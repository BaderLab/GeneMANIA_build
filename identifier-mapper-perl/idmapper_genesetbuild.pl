...
foreach my $id (@stable_ids) {	
		...
		given ($dbname) {
			...
			when (/Uniprot/) {
				if (defined $uniprot_id) { 
					if(!($uniprot_id =~ /$xdb->primary_id/)){
						$uniprot_id = $uniprot_id . ";" . $xdb->primary_id;
					}
				} else {
	...
	print "uniprot id - regular method: [$uniprot_id]\n";
	#if ($organism eq "mus_musculus") {
	#	print "mgi id: [$mgi_id]\n";
	#}
	...
	if (! defined $gene_name) {
		...
		#There are missing mapping.  If there is a gene names try and get additional
		# identifiers using the gene name.
		my $gene = $gene_adaptor->fetch_by_display_label($gene_name);
		print "Current gene:" . $gene_name . "\n";
		print "Gene:" . $gene->stable_id() . "\n";
		my @xdb_list = @{ $gene->get_all_DBEntries() };
		foreach my $xdb (@xdb_list) {
			my $dbname = $xdb->dbname;
			my $realname = "";
			...
			given ($dbname) {
				...
				when (/Uniprot/) {
					print "1st About to add: [" . $xdb->primary_id . "]\n";
					print "But maybe should add: [" . $xdb->display_id() . "]\n";
					if (defined $uniprot_id) { 
						if(!($uniprot_id =~ /$xdb->primary_id/)){
							$uniprot_id = $uniprot_id . ";" . $xdb->primary_id;
						}
				...
				when (/UniProt/) {
					print "About to add: [$xdb->primary_id]\n";
					if (defined $uniprot_id) { 
						if(!($uniprot_id =~ /$xdb->primary_id/)){
							$uniprot_id = $uniprot_id . ";" . $xdb->primary_id;
						}
	...
	print "uniprot id - expanded list: [$uniprot_id]\n";
