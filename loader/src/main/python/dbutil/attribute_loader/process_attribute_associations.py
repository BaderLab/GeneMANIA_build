...
import logging, os
import unittest, test_support, tempfile, process_attributes, process_identifiers, shutil
    ...
    def lookup_node_id(self, gene):
        ...
        try:
            ...
            try:
                ...
                if len(records) > 1:
                    logger.warn("multiple ids '%s', using the first one" % gene)
                    ID = int(records[0][0])
...
#        print sql
        ...
        try:
            ...
            try:
                records = cursor.fetchall()
                if len(records) > 1:
                    logger.warn("multiple ids for attribute '%s', using the first one" % attribute)
                    ID = int(records[0][0])
